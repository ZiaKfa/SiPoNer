...
<div class="container align-items-center justify-content-center">
  ...
  <form action="<?= base_url('mypokemon/update'); ?>" method="post">
    <div class="mb-3">
      <label for="name" class="form-label">Nickname</label>
        <input type="text" class="form-control" id="name" name="name" value="<?= $mypokemon['name']; ?>">
    </div>
    <div class = "mb-3">
        ...
        <select class="form-select" id="pokemon_id" name="pokemon_id">
            <?php foreach($pokemon as $p) : ?>
                <option value="<?= $p['id'] ?>" 
                <?php if($p['id'] == $mypokemon['pokemon_id']) : ?> selected <?php endif; ?>
                ><?= $p['name'] ?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <input type="hidden" name="trainer_id" value ="<?= $this->session->userdata('id') ?>">
    <input type="hidden" name="oldname" value ="<?= $mypokemon['name'] ?>">
